...
const SubjectsSection = ({ enrolledSubjects, loadingSubjects, attendanceData, loadingAttendance, fetchAttendance }) => {
    return (
                                                    ...
                                                    <div className="mt-4">
                                                        ...
                                                        <div className="space-y-2 max-h-48 overflow-y-auto">
                                                            {attendance.records.slice(0, 10).map((record, idx) => (
                                                                <div key={idx} className={`flex items-center justify-between p-2 rounded-lg text-sm ${record.status === 'present' ? 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300' : 'bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-300'}`}>
                                                                    <span className="font-medium">{record.date || record.attendance_date || 'N/A'}</span>
                                                                    <span className={`px-2 py-1 rounded text-xs font-medium ${record.status === 'present' ? 'bg-green-200 dark:bg-green-800 text-green-800 dark:text-green-200' : 'bg-red-200 dark:bg-red-800 text-red-800 dark:text-red-200'}`}>{record.status === 'present' ? 'Present' : 'Absent'}</span>
                                                                </div>
                                                            ))}
                                                        </div>
                                                    </div>
